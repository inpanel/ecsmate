...
class Install(object):
    ...
    def __init__(self):
        ...
        self.installpath = '/usr/local/ecsmate'
    ...
    def install_ecsmate(self):
        localpkg_found = False
        if os.path.exists(os.path.join(os.path.dirname(__file__), 'ecsmate.tar.gz')):
            # local install package found
            localpkg_found = True
        else:
            # or else install online
            print '* Downloading install package from www.ecsmate.org'
            f = urllib2.urlopen('http://www.ecsmate.org/api/latest')
            data = f.read()
            f.close()
            downloadurl = re.search('"download":"([^"]+)"', data).group(1).replace('\/', '/')
            self._run('wget -nv -c "%s" -O ecsmate.tar.gz' % downloadurl)
            
        # uncompress and install it
        self._run('mkdir ecsmate')
        self._run('tar zxmf ecsmate.tar.gz -C ecsmate')
        if not localpkg_found: os.remove('ecsmate.tar.gz')
        
        # stop service
        print
        if os.path.exists('/etc/init.d/ecsmate'):
            self._run('/etc/init.d/ecsmate stop')

        # backup data and remove old code
        if os.path.exists('%s/data/' % self.installpath):
            self._run('cp -r %s/data/* ecsmate/data/' % self.installpath, True)
        self._run('rm -rf %s' % self.installpath)
        
        # install new code
        self._run('mv ecsmate %s' % self.installpath)
        self._run('chmod +x %s/config.py %s/server.py' % (self.installpath, self.installpath))
        
        # install service
        initscript = '%s/tools/init.d/%s/ecsmate' % (self.installpath, self.distname)
        self._run('cp %s /etc/init.d/ecsmate' % initscript)
        self._run('chmod +x /etc/init.d/ecsmate')
        
        # start service
        if self.distname in ('centos', 'redhat'):
            self._run('chkconfig ecsmate on')
            self._run('service ecsmate start')
        elif self.distname == 'ubuntu':
            pass
        elif self.distname == 'debian':
            pass
    ...
    def install(self):
        ...
        # get the latest ECSMate version
        print '* Installing latest ECSMate'
        self.install_ecsmate()
        ...
        print 
        print '* The URL of your ECSMate is:',
        print 'http://%s:8899/' % self.detect_ip()
        print 
