...
function($scope, $rootScope, $location, Module, Message, Request){
	...
	$scope.load = function(){
		Request.get('/account?status=enable', function(data){
			if (data.code == 0) {
				$scope.accounts = data.data;
				if ($scope.accounts.length > 0) {
					if (!$rootScope.$account.access_key_id)
						$rootScope.$account.access_key_id = $scope.accounts[0].access_key_id;
					$scope.loadinstances();
				}
			}
			$scope.loaded = true;
	...
	$scope.loadinstances = function(pagedir){
		...
		var query = 'access_key_id='+$rootScope.$account.access_key_id+'&page_number='+$rootScope.$ecs.page_number+'&page_size='+$rootScope.$ecs.page_size;
...
function($scope, $rootScope, $routeParams, $location, Module, Message, Request, Timeout, Backend){
		...
		Request.get('/ecs/instance?'+query, function(data){
			...
			if ($scope.instance) {
				...
				if (s == 'pending' || s == 'starting' || s == 'stopping' || s == 'transferring' || s == 'resetting')
					Timeout(function(){$scope.load(true);}, 1000, module);
	...
	$scope.stop = function(){
		...
		$scope.confirm = function(){dostop();};
		$scope.confirm_force = function(){dostop(true);};
		$('#confirm').modal();
	};
	...
	$scope.reboot = function(){
		...
		$scope.confirm = function(){doreboot();};
		$scope.confirm_force = function(){doreboot(true);};
		$('#confirm').modal();
	};
	...
	$scope.createsnapshot = function(){
		...
		$scope.confirm = function(){docreatesnapshot();};
	...
	$scope.cancelsnapshot = function(snapshot){
		...
		$scope.confirm = function(){docancelsnapshot();};
		$scope.confirm_forcebutton = false;
		$scope.confirm_force = false;
		$('#confirm').modal();
	};
	...
	$scope.deletesnapshot = function(snapshot){
		...
		$scope.confirm = function(){dodeletesnapshot();};
		$scope.confirm_forcebutton = false;
		$scope.confirm_force = false;
		$('#confirm').modal();
	};
	...
	$scope.rollbacksnapshot = function(snapshot){
		...
		$scope.confirm = function(){dorollbacksnapshot();};
		$scope.confirm_forcebutton = false;
		$scope.confirm_force = false;
		$('#confirm').modal();
	};
	...
	$scope.loadvpsmate = function(quiet){
	...
	$scope.vpsmate_install = function(){
		...
		$scope.sshconfirm_title = '安装/升级 VPSMate 到此服务器上';
		$scope.sshconfirm_button = '安装或升级';
		$scope.sshconfirm = doinstallvpsmate;
		$('#sshconfirm').modal();
	};
	var doinstallvpsmate = function(){
		$scope.processing = true;
		Message.setInfo('正在安装 VPSMate 到远程服务器，请稍候...');
		Backend.call(
			$scope,
			module,
			'/backend/vpsmate_install',
			'/backend/vpsmate_install_'+$scope.ssh_ip,
			{
				'ssh_ip': $scope.ssh_ip,
	...
	$scope.vpsmate_uninstall = function(){
		if (!$scope.ssh_ip) $scope.ssh_ip = $scope.instance.PublicIpAddress.AllocateIpAddress;
		if (!$scope.ssh_port) $scope.ssh_port = '22';
		if (!$scope.ssh_user) $scope.ssh_user = 'root';
		$scope.sshconfirm_title = '在此服务器上卸载 VPSMate';
		$scope.sshconfirm_button = '开始卸载';
		$scope.sshconfirm = douninstallvpsmate;
		$('#sshconfirm').modal();
	};
	var douninstallvpsmate = function(){
		$scope.processing = true;
		Message.setInfo('正在从远程服务器卸载 VPSMate，请稍候...');
		Backend.call(
			$scope,
			module,
			'/backend/vpsmate_uninstall',
			'/backend/vpsmate_uninstall_'+$scope.ssh_ip,
			{
				'ssh_ip': $scope.ssh_ip,
				'ssh_port': $scope.ssh_port,
				'ssh_user': $scope.ssh_user,
	...
	$scope.syncaccesskey = function(){
		...
		$scope.sshconfirm = doupdatevpsmate;
		$('#sshconfirm').modal();
	};
	var doupdatevpsmate = function(){
		$scope.processing = true;
		Message.setInfo('正在同步密钥到远程，请稍候...');
		Backend.call(
			$scope,
			module,
			'/backend/vpsmate_update',
			'/backend/vpsmate_update_'+$scope.ssh_ip,
			{
				'ssh_ip': $scope.ssh_ip,
				'ssh_port': $scope.ssh_port,
				'ssh_user': $scope.ssh_user,
